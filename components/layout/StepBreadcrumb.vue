      ...
      <ol class="flex items-center justify-center gap-0">
        <li v-for="(step, index) in steps" :key="step.id" class="flex items-center">
          ...
          <div class="flex flex-col items-center gap-1 relative">
            <!-- Lingkaran step -->
            <button class="w-8 h-8 sm:w-9 sm:h-9 rounded-full flex items-center justify-center
                     font-fredoka font-bold text-sm transition-all duration-300 border-2"
              :class="getStepClass(step.id)" @click="handleStepClick(step.id)" :disabled="!canNavigate(step.id)"
              :title="step.label" :aria-label="`Step ${step.id}: ${step.label}`"
            ...
            <!-- Label step (desktop) -->
            <span class="hidden sm:block text-xs font-nunito font-semibold text-center
          ...
          <div v-if="index < steps.length - 1" class="h-0.5 w-8 sm:w-16 mx-1 rounded-full transition-all duration-500"
            :class="isDone(step.id) ? 'bg-jungle' : 'bg-gray-200'" aria-hidden="true" />
        </li>
      </ol>
    </div>
  </nav>
...
// Cek apakah step bisa di-navigate (step sebelumnya sudah selesai)
function canNavigate(stepId) {
  if (stepId === 1) return true
  return store.isStepDone(stepId - 1)
}
